...
oceanbase::mergeserver::ObMergeJoinAgent::ObMergeJoinAgent(ObMergerRpcProxy &proxy)
:scan_agent_(proxy),get_agent_(proxy)
{
  req_agent_ = NULL;
}

oceanbase::mergeserver::ObMergeJoinAgent::~ObMergeJoinAgent()
{
  clear();
  req_agent_ = NULL;
}

int oceanbase::mergeserver::ObMergeJoinAgent::set_request_param(const ObScanParam &scan_param, 
...
int oceanbase::mergeserver::ObMergeJoinAgent::set_request_param(const ObGetParam &get_param, 
                                                                ObUPSCellStream &ups_stream, 
                                                                ObUPSCellStream &ups_join_stream, 
                                                                const ObSchemaManagerV2 &schema_mgr, 
                                                                const int64_t max_memory_size)
...
bool oceanbase::mergeserver::ObMergeJoinAgent::is_request_fullfilled()
{
  bool result = false;
  if (NULL != req_agent_)
  {
