...
namespace oceanbase
{
  ...
  namespace mergeserver
  {
    ...
    private:
      const oceanbase::common::ObSchemaManagerV2 *schema_mgr_;
      ...
      oceanbase::common::ObGetParam fake_get_param_;
      const oceanbase::common::ObScanParam *scan_param_;
      const oceanbase::common::ObGetParam  *get_param_;
      /// @property when get, req_param_ == get_param_; when scan req_param_ == & fake_get_param_
      const oceanbase::common::ObGetParam *req_param_;

    private:
      ObMergerTabletLocation                cur_cs_addr_;
      oceanbase::common::ObStringBuf        cur_row_key_buf_;
      /// properties for get request
      oceanbase::common::ObMerger           merger_;
      bool                                  merger_iterator_moved_;
      oceanbase::common::ObGetParam         cur_get_param_;
      int64_t                               got_cell_num_;
      /// properties for scan request
      oceanbase::common::ObScanParam        cur_scan_param_;
      oceanbase::common::ObMemBuffer        cs_scan_buffer_; 
      oceanbase::common::ObMemBuffer        ups_scan_buffer_; 
      oceanbase::common::ObScanner          cur_cs_result_;
      ObMergerRpcProxy*                     rpc_proxy_;
      bool                                  param_contain_duplicated_columns_;
      int32_t                               column_id_idx_map_[oceanbase::common::OB_MAX_COLUMN_NUMBER];
      /// properties for join
      int64_t cur_rpc_offset_beg_;
      oceanbase::common::ObReadParam        cur_join_read_param_;
      bool                                  is_need_query_ups_;
      bool                                  is_need_join_;
      ObCellArray                           join_param_array_;
      oceanbase::common::ObVector<int64_t>  join_row_width_vec_;
      oceanbase::common::ObVector<int64_t>  join_offset_vec_;

    private:
      int do_merge_join(); 
      int merge();  
      int merge_next_row(int64_t &cur_row_beg, int64_t &cur_row_end);
      int apply_whole_row(const oceanbase::common::ObCellInfo &cell, const int64_t row_beg);
    ...
    class ObMergeJoinAgentImp :public oceanbase::common::ObIterator
    {
    public:
      virtual ~ObMergeJoinAgentImp()
      {
      }
      int get_cell(oceanbase::common::ObCellInfo * *)
      {
        return oceanbase::common::OB_SUCCESS;
      }
      int get_cell(oceanbase::common::ObCellInfo * *, bool *)
      {
        return oceanbase::common::OB_SUCCESS;
      }
      int next_cell()
    ...
    private:
      ...
      const oceanbase::common::ObScanParam *param_;
      ObMergeJoinOperator merge_join_operator_;
      ...
      oceanbase::common::ObCellArray::OrderDesc orderby_desc_[oceanbase::common::OB_MAX_COLUMN_NUMBER];
    };
  }
}
