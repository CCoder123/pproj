...
namespace oceanbase 
{
  namespace common 
  {
    ...
    const char* const ObSchema::get_compress_func_name() const
    {
    ...
    bool ObSchemaManagerV2::parse_from_file(const char* file_name, tbsys::CConfig& config) 
    {
      ...
      if (parse_ok)
      {
        ...
        else
        {
          ...
          {
            TBSYS_LOG(ERROR, "%d error table number", table_nums_);
            parse_ok = false;
            table_nums_ = 0;
          }
          // app section and sections.size() - 1 tables
    ...
    bool ObSchemaManagerV2::parse_one_table(const char* section_name, tbsys::CConfig& config, ObTableSchema& schema) 
    {
      ...
      if (parse_ok)
      {
        ...
        if (max_column_id < 1)
        {
          TBSYS_LOG(ERROR, "max_column_id is %llu", max_column_id);
          parse_ok = false;
        }
        if (max_column_id > MAX_ID_USED)
        {
          TBSYS_LOG(ERROR, "we limit our column id less than %llu", MAX_ID_USED);
          parse_ok = false;
        }
        if (rowkey_max_length <= 0 || rowkey_split < 0 ) 
        {
    ...
    bool ObSchemaManagerV2::parse_column_info(const char* section_name, tbsys::CConfig& config, ObTableSchema& schema) 
    {
          ...
          it != column_info_strs.end() && parse_ok; ++it)
      {
        ...
        if (parse_ok)
        {
          ...
          if (maintained != 0 && maintained != 1)
          {
            TBSYS_LOG(ERROR, "maintained %d is unacceptable", maintained);
            parse_ok = false;
          }
          if (maintained == 1) 
          {
        ...
        if (parse_ok)
        {
          ...
          if (id == 0 )
          {
            TBSYS_LOG(ERROR, "column id error id = %d", id);
            parse_ok = false;
          }
        }
