...
/* public function */
extern header *http_header_parse(const char *buf, size_t size); 
extern void header_free(header *header);
extern size_t headers_end(const char *mime, size_t l);
extern int http_method_id_get(const char *m);
extern header_entry *header_entry_get(const header *h, int id);
extern void method_free(struct method_t *m);
extern char *http_reply_build(header *h, int major, int minor, enum http_status statu);
extern void http_header_entry_append(header *h, char *name, char *value);
extern const char *http_status_string(enum http_status status);
extern header * header_ref(header *h) ;
extern header * header_unref(header *h) ;
#endif
